...
class ReprogrammingLayer(nn.Module):
    ...
    def reprogramming(self, target_embedding, source_embedding, value_embedding):
        ...
        """
            ...
            blhe表示target_embedding张量的维度顺序，即batch_size、seq_len、num_head和embedding_size。
            she表示source_embedding张量的维度顺序，即source_length(词的个数)、num_head和embedding_size。
            ->箭头后面的bhls表示运算结果的维度顺序，即batch_size、num_head、seq_len和source_length(词的个数)。
            einsum函数会根据给定的维度顺序和求和规则，计算两个张量之间的运算结果。

            这段代码的用途是计算target_embedding和source_embedding之间的相似度或相关性。
            具体来说，它计算了每个target_embedding和每个source_embedding之间的点积，并将结果存储在scores张量中。
            这个张量的形状为(batch_size, hidden_size, length, source_length)，其中每个元素表示对应target_embedding和source_embedding之间的相似度或相关性。
        """
        scores = torch.einsum("blhe,she->bhls", target_embedding, source_embedding)

        """
            做完attention后得到注意力分数reprogramming_embedding   Attention(Q,K,V) = softmax(QK^T/√d_k)·V
        """
