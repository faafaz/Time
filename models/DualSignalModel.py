...
from utils.AFFM import AdaptiveFrequencyFilter
from models.iTransformer import Model as iTransformer
from models.TimeXer import Model as TimeXer


class Model(nn.Module):
    """双信号分离模型: 使用AFFM自适应频率滤波模块分离信号, 分别用iTransformer建模, 最后加权融合"""

    def __init__(self, configs, hidden_dim=128, model_name=["iTransformer", "TimeXer"]):
        ...
        # 使用AFFM自适应频率滤波模块
        self.affm = AdaptiveFrequencyFilter(
            seq_len=configs.seq_len,
            enc_in=configs.n_input_features,
            hidden_dim=hidden_dim,
            rfft=True
        )
 
        # 创建两个iTransformer模型
        self.main_freq_model = self._create_model(model_name[0])
        self.residual_model = self._create_model(model_name[1])
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # 1. 信号分离: 使用AFFM自适应提取主频率成分和残差信号
        residual, main_freq, filter_coeff = self.affm(x_enc)

        # 2. 分别用两个模型进行预测
        # 主频率成分预测
        pred_main = self.main_freq_model(main_freq, x_mark_enc, x_dec, x_mark_dec, mask)

        # 残差信号预测
        pred_residual = self.residual_model(residual, x_mark_enc, x_dec, x_mark_dec, mask)
