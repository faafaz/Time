...
class Dataset_power_minute(Dataset):
    def __init__(self, model_name, data_path_list, flag, size=None, target='温度/气温', scale=True, freq='60min',
                 date_name="时间",
                 get_pred_type="first"):
        ...
        # 根据flag设置不同的步长和总长度
        if (self.flag == 'test' or self.flag == 'val') and get_pred_type == "all":
            # 计算测试和验证时的总预测次数
    ...
    def __read_data__(self):
        ...
        if self.scale:
            ...
            data = df_data[[self.target]].values
...
def get_data_loader(args, flag, shuffle_flag=True, drop_last=False):
    ...
    if flag == 'train':
        ...
        shuffle_flag = False  # 只有测试的时候要关闭shuffle
