...
#         # 四季月份映射（可通过 config['season_months'] 覆盖）
#         self.season_months = config.get('season_months', {'春':[3,4,5], '夏':[6,7,8], '秋':[9,10,11], '冬':[12,1,2]})
...
#             # 全局排除区站号为 57484 的数据
#             if self.station_id_column in df_clean.columns:
#                 sid_str = df_clean[self.station_id_column].astype(str).str.strip().str.replace('.0', '', regex=False)
#                 drop_mask = sid_str == '57484'
#                 drop_cnt = int(drop_mask.sum())
#                 if drop_cnt > 0:
#                     self.logger.info(f"年份 {year} | 站点 57484 排除 {drop_cnt} 行")
...
#         # 先清理输出目录下旧的子目录与 csv/txt（保留 processing.log）
#         try:
...
#                     elif entry.is_file():
#                         if entry.name.endswith('.csv') or entry.name.endswith('_stats.txt'):
#                             try:
#                                 entry.unlink()
#                             except Exception:
#                                 pass
...
#         # 保存“全年”到子目录
#         whole_dir = self.output_dir / '全年'
#         whole_dir.mkdir(parents=True, exist_ok=True)
#         for dataset_name, dataset in datasets.items():
#             ds = dataset.copy()
#             if '年份' in ds.columns:
#                 ds = ds.drop(columns=['年份'])
#             if self.time_column in ds.columns and np.issubdtype(ds[self.time_column].dtype, np.datetime64):
#                 ds[self.time_column] = ds[self.time_column].dt.strftime('%Y-%m-%d %H:%M:%S')
#             out_file = whole_dir / f"{dataset_name}_dataset.csv"
#             ds.to_csv(out_file, index=False, encoding='utf-8-sig')
#             self.logger.info(f"保存 全年-{dataset_name}: {out_file}")
#             self.generate_dataset_stats(ds, f"全年_{dataset_name}")
...
#                 if df2.empty:
#                     # 写出空结构文件以保持目录完整
#                     if '年份' in df2.columns:
#                         df2 = df2.drop(columns=['年份'])
#                     out_file = season_dir / f"{dataset_name}_dataset.csv"
#                     df2.to_csv(out_file, index=False, encoding='utf-8-sig')
#                     self.generate_dataset_stats(df2, f"{season}_{dataset_name}")
#                     continue
#                 df2[self.time_column] = pd.to_datetime(df2[self.time_column])
#                 df2['__month__'] = df2[self.time_column].dt.month
#                 season_df = df2[df2['__month__'].isin(months)].drop(columns=['__month__'])
#                 if '年份' in season_df.columns:
#                     season_df = season_df.drop(columns=['年份'])
#                 if self.time_column in season_df.columns and np.issubdtype(season_df[self.time_column].dtype, np.datetime64):
#                     season_df[self.time_column] = season_df[self.time_column].dt.strftime('%Y-%m-%d %H:%M:%S')
#                 out_file = season_dir / f"{dataset_name}_dataset.csv"
#                 season_df.to_csv(out_file, index=False, encoding='utf-8-sig')
#                 self.logger.info(f"保存 {season}-{dataset_name}: {out_file}")
#                 self.generate_dataset_stats(season_df, f"{season}_{dataset_name}")
...
def process_wind_data(file_path, output_dir=r'dataset\cur_dataset\wind_farm\farm2'):
    # 1. 加载数据
    print("正在加载数据...")
    df = pd.read_csv(file_path)

    # 2. 筛选列：保留时间、所有风速（windspeed）和功率（Power）
    windspeed_cols = [col for col in df.columns if 'windspeed' in col.lower()]
    required_cols = ['Time'] + windspeed_cols + ['Power']
    df = df[required_cols]
    ...
    print(f"原始数据共 {original_count} 行，去重后剩余 {len(df)} 行。")
    ...
    if df.isnull().sum().sum() > 0:
        ...
        df = df.dropna()
        print(f"插值并剔除大段缺失后，剩余 {len(df)} 行数据。")

    # 5. 时间格式转化
    df['Time_Formatted'] = df['Time'].apply(lambda x: f"{x.year}/{x.month}/{x.day} {x.hour}:{x.minute:02d}")
    ...
    for season in seasons:
        ...
        if not os.path.exists(season_dir):
            os.makedirs(season_dir) # 递归创建目录
            print(f"已创建目录: {season_dir}")

        # 筛选数据
        train_df = df[(df['Year'] == 2019) & (df['Season'] == season)].copy()
        test_df = df[(df['Year'] == 2020) & (df['Season'] == season)].copy()

        final_cols = ['Time_Formatted'] + windspeed_cols + ['Power']
        train_output = train_df[final_cols].rename(columns={'Time_Formatted': 'Time'})
        test_output = test_df[final_cols].rename(columns={'Time_Formatted': 'Time'})

        # 保存为CSV
        train_path = os.path.join(season_dir, 'train.csv')
        test_path = os.path.join(season_dir, 'test.csv')

        train_output.to_csv(train_path, index=False)
        test_output.to_csv(test_path, index=False)

        print(f"--- {season} 数据集生成完毕 ---")
        print(f"训练集(2019): {len(train_output)} 条 | 测试集(2020): {len(test_output)} 条")

# 调用函数
process_wind_data(r'dataset\china_wind_power\data_processed\wind_farms\Wind farm site 2.csv')